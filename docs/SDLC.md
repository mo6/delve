...
| **Test** | `./run-tests.sh` (pytest, ruff incl. `S`, `pip-audit`, `./tools.sh issues --check`, `delve validate`), plus the in-session `/code-review` and `/security-review` skills; screen look on demand via `./tools.sh screenshot` | everyone, as the Definition of Done |
...
2. **`gate-check` skill and the `screenshot-nudge` hook** (§5, §6). Both wrap existing tools (`tools/screenshot.py`, `tools/issues.py --check`, `./run-tests.sh`) around gotchas CLAUDE.md already documents in prose. Converting a remembered convention into an enforced one is the highest payoff for the lowest design risk in this list.
